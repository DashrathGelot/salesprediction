...
def createGraph(request):
    ...
    if request.POST:
        bills_requested = SK_Bills.objects.filter(store_person=StoreDetails.objects.get(
            StoreName=request.POST["store"]), pd_nm=request.POST["product"]).order_by('-date_data')
        ...
        return render(request, 'admin/newGraph.html', {'no_data': no_date, 'store': bills_store, 'product': bills_products, 'plot_div': plot_div, 'data': data})
...
def GraphCall(request):
    ...
    print(data)
    # data = dumps(data)
    return render(request, 'grap.html', {'data': data, 'sks': sk_nm, 'tots': tots})
